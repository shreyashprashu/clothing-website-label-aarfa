...
The React app lives in a **single file** ([src/App.jsx](src/App.jsx), ~2700 lines) — deliberate. Sections are demarcated by big banner comments:

```
PRODUCTS — product catalog (sizes XS–XXL, no colors)
...
HEADER, AnnouncementBar, IntlPricingNotice (non-INR persistent strip), MOBILE MENU, HERO, PRODUCT CARD
HOMEPAGE SECTIONS (CategoryPreview, EditorialBanner, SaleStrip, ValueProps, Newsletter)
CATEGORY PAGE, PRODUCT PAGE (with intl-fee notice block under price)
CART DRAWER, SEARCH OVERLAY, AUTH MODAL (Supabase email OTP + Google OAuth, no phone OTP)
WISHLIST, CHECKOUT (Razorpay only, no COD), ABOUT, CONTACT
...
| [public/images/products/](public/images/products/) | 51 source JPEGs + 153 WebP variants (`.400.webp`, `.800.webp`, `.1280.webp`). Mobile cards download ~30 KB instead of ~108 KB. |
...
1. **`scripts/optimize-images.mjs`** reads every `.jpe?g` / `.png` under `public/images/products/` and writes `<name>.400.webp`, `<name>.800.webp`, `<name>.1280.webp` next to the original via `sharp` at quality 82. Idempotent on mtime — re-runs are cheap.
...
- Image pipeline (sharp → WebP variants) + `<ProductImage>` responsive helper.
